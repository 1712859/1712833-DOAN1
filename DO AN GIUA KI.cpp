...
struct sinhvien {
	...
	wchar_t sothich3[1001];

};
typedef struct sinhvien SV;

void TACHCHUSO(FILE *input, int &n)
...
void DOC_1_SINHVIEN(FILE* fp, SV& x)

{

	DOCTAPTIN(x.MSSV, fp);

	DOCTAPTIN(x.HOVATEN, fp);

	DOCTAPTIN(x.khoa, fp);

	TACHCHUSO(fp, x.KHOA);

	DOCTAPTIN(x.ngaysinh, fp);

	DOCTAPTIN(x.hinh, fp);

	DOCTAPTIN(x.mota, fp);

	DOCTAPTIN(x.sothich1, fp);
